...
@section("content")
    ...
    <div class="fixed inset-0 z-10 hidden overflow-y-auto" id="createBotModal">
            ...
            >
                ...
                >
                    <button
                        ...
                        data-dismiss="modal"
                        aria-label="Close"
                        onclick="document.getElementById('createBotModal').classList.add('hidden')"
                    >
            ...
            >
                ...
                >
                    <button
                        ...
                        data-dismiss="modal"
                        aria-label="Close"
                        onclick="document.getElementById('activateBotModal').classList.add('hidden')"
                    >
                        <span aria-hidden="true" class="text-[20px]">
                            &times;
                        </span>
                ...
                <div class="bg-white px-4 py-5 sm:p-6">
                    ...
                    <button
                        id="activateBotButton"
                        class="text-md rounded bg-blue-500 px-3 py-1 text-white"
                    >
                        Activate
                    </button>
                </div>
                ...
                try {
                    ...
                    response.forEach(function (bot) {
                        const activeButton =
                            bot.status === '1'
                                ? ''
                                : `<button class="flex-1 bg-blue-500 text-white py-1 px-2 rounded activate-btn text-[13px]" onClick="openActivateBotModal(${bot.id})">Activate</button>`;
                        const status =
                            bot.status === '1'
                                ? '<span class="inline-block px-2 py-1 text-xs font-semibold text-green-800 bg-green-100 rounded-full">Bật</span>'
                                : '<span class="inline-block px-2 py-1 text-xs font-semibold text-gray-800 bg-gray-100 rounded-full">Tắt</span>';

                        document
                            .getElementById('botTableBody')
                            .insertAdjacentHTML(
                                'beforeend',
                                `
                            <tr>
